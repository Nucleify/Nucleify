...
import type { DialogVisibleInterface, UseDialogInterface } from 'atomic'

export function useAtomicDialog(): UseDialogInterface {
  const selectedObject: Ref<ObjectType> = ref()
  const visible: DialogVisibleInterface = {
    create: ref(false),
    delete: ref(false),
    edit: ref(false),
    show: ref(false),
